...
%% we enable off-heap message queue data.
%% Sebastien: Removed fullsweep for now because we now have
%% a lot of live data for the buffers.
...
dist_cntrlr_setup_loop(Socket, TickHandler, Sup) ->
    receive
	    ...
	    flush_controller(InputHandler, Socket),
            ...
            dist_cntrlr_output_setup(DHandle, Socket)
    end.
...
dist_cntrlr_input_setup(DHandle, Socket, Sup) ->
    ...
    receive
        DHandle ->
            ...
            dist_cntrlr_input_loop(DHandle, Socket, Rcv, 0)
    end.

dist_cntrlr_input_loop(DHandle, Socket, Rcv, N) when N =< ?ACTIVE_INPUT/2 ->
    inet:setopts(Socket, [{active, ?ACTIVE_INPUT - N}]),
    dist_cntrlr_input_loop(DHandle, Socket, Rcv, ?ACTIVE_INPUT);
...
dist_cntrlr_send_data(DHandle, Socket, Snd) ->
    dist_cntrlr_send_data(DHandle, Socket, Snd, false).

dist_cntrlr_send_data(DHandle, Socket, Snd, More) ->
    case erlang:dist_ctrl_get_data(DHandle) of
        ...
        Data ->
            ...
            dist_cntrlr_send_data(DHandle, Socket, Snd2, More1)
    end.

dist_cntrlr_send_next(Socket, Snd) ->
    case packet_sender:next(Snd) of
        {empty, Snd1} -> {false, Snd1};
        {Packet, Snd1} ->
            sock_send(Socket, Packet),
            {true, Snd1}
    end.

dist_cntrlr_send_all(Socket, Snd) ->
    case dist_cntrlr_send_next(Socket, Snd) of
        {true, Snd1} -> dist_cntrlr_send_all(Socket, Snd1);
        {false, Snd1} -> Snd1
    end.
...
dist_cntrlr_output_blocking_loop(DHandle, Socket, Snd) ->
    receive
        dist_data ->
            dist_cntrlr_send_data_loop(DHandle, Socket, Snd);
        {send, From, Ref, Data} ->
            dist_cntrlr_send_from_loop(DHandle, Socket, Snd, From, Ref, Data);
        _ ->
            %% Drop garbage message...
            dist_cntrlr_output_blocking_loop(DHandle, Socket, Snd)
    end.

dist_cntrlr_output_consuming_loop(DHandle, Socket, Snd) ->
    receive
        dist_data ->
            dist_cntrlr_send_data_loop(DHandle, Socket, Snd);
        {send, From, Ref, Data} ->
    ...
    after
        0 ->
            dist_cntrlr_send_next_and_loop(DHandle, Socket, Snd)
    end.

dist_cntrlr_send_data_loop(DHandle, Socket, Snd) ->
    %% Outgoing data from this node...
    try dist_cntrlr_send_data(DHandle, Socket, Snd) of
        {More, Snd1} -> dist_cntrlr_send_loop(DHandle, Socket, Snd1, More)
    catch _ : _ -> death_row()
    end.

dist_cntrlr_send_next_and_loop(DHandle, Socket, Snd) ->
    %% Outgoing data from this node...
    try dist_cntrlr_send_next(Socket, Snd) of
        {More, Snd1} -> dist_cntrlr_send_loop(DHandle, Socket, Snd1, More)
    catch _ : _ -> death_row()
    end.
...
dist_cntrlr_send_from_loop(DHandle, Socket, Snd, From, Ref, Data) ->
    ...
    Ch = packet_channel:channel(Data),
    Snd1 = packet_sender:schedule(Ch, Data, Snd),
    Snd2 = dist_cntrlr_send_all(Socket, Snd1),
    From ! {Ref, ok},
    dist_cntrlr_output_blocking_loop(DHandle, Socket, Snd2).

dist_cntrlr_send_loop(DHandle, Socket, Snd, true) ->
    dist_cntrlr_output_consuming_loop(DHandle, Socket, Snd);
dist_cntrlr_send_loop(DHandle, Socket, Snd, false) ->
    dist_cntrlr_output_blocking_loop(DHandle, Socket, Snd).
...
% Sebastien: Removed because death_row/1 is never called with `normal`
% death_row(normal) ->
